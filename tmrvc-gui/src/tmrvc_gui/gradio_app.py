...
from tmrvc_core.constants import SERVE_PORT, GUI_PORT, SAMPLE_RATE
from tmrvc_gui.i18n import (
    get_head_js,
    get_locales_js,
    available_languages,
    get_tooltip,
    ...
    with gr.Blocks() as tab:
        ...
        with gr.Row():
            manifest_path = gr.Textbox(
                ...
                max_lines=1,
                info="Path to the curation manifest file (JSONL format)",
            )
        ...
        with gr.Row():
            selected_record_id = gr.Textbox(label="Selected Record ID", max_lines=1)
        ...
        with gr.Row():
            ...
            actor_id = gr.Textbox(label="Your ID", value="reviewer_1", max_lines=1)
        ...
        def on_select(evt: gr.SelectData, rows):
            ...
            text = (
                ...
                if details
                else rows.iloc[evt.index[0]]["transcript"]
            )
        ...
        def do_promote(record_id, version, role, aid, reason):
            ...
            return (
                f"Promoted {record_id} (new version: {resp.get('metadata_version')})."
            )
    ...
    with gr.Blocks() as tab:
        ...
        def do_ingest(files, dir_path, legal):
            ...
            _audit.log(
                ...
                "gradio",
                "ingest",
                after_state=f"legality={legal}, sources={len(sources)}",
            )
...
def _build_evaluation_arena() -> gr.Blocks:
    ...
    with gr.Blocks() as tab:
        ...
        current_assignment_id = gr.Textbox(label="Assignment ID", interactive=False)
        ...
        def create_session(name, n_assignments, evaluator_id):
            ...
            if resp is None:
                return (
                    ...
                    [],
                    0,
                    {"error": "Failed to create session. Is tmrvc-serve running?"},
                )
        ...
        btn_create_session.click(
            ...
            outputs=[
                session_id_state,
                assignment_ids_state,
                assignment_idx_state,
                session_info,
            ],
        )
        ...
        def get_next_assignment(session_id, assignment_ids, idx):
            ...
            if resp is None:
                return (
                    ...
                    "",
                    assignment_id,
                    f"Assignment {assignment_id} (API fetch failed)",
                )
            text = resp.get("text", "")
            return (
                idx + 1,
                ...
                text,
                assignment_id,
                f"Assignment {idx + 1}/{len(assignment_ids)}",
            )
        ...
        def record_preference(pref, pair_id, ma, mb, role, rid, notes):
            ...
            submit_body = {
                "rating": float(
                    ma if pref == "A" else mb if pref == "B" else (ma + mb) / 2
                ),
    ...
    with gr.Blocks() as tab:
        ...
        with gr.Tabs():
            ...
            with gr.Tab("Reference Trimming"):
                ...
                trim_manifest = gr.File(label="manifest.jsonl", file_types=[".jsonl"])
                ...
                trim_log = gr.Textbox(label="Log", interactive=False, lines=10)
                ...
                def _run_trim(manifest_file, audio_dir, output_dir):
                    ...
                    sys.path.insert(
                        0, str(Path(__file__).resolve().parents[3] / "scripts" / "eval")
                    )
                    try:
            ...
            with gr.Tab("Control Sweeps"):
                ...
                sweep_manifest = gr.File(label="manifest.jsonl", file_types=[".jsonl"])
            ...
            with gr.Tab("Rater Assignment"):
                ...
                assign_manifest = gr.File(label="manifest.jsonl", file_types=[".jsonl"])
                with gr.Row():
                    assign_num_raters = gr.Number(
                        label="Number of Raters", value=30, precision=0
                    )
                    assign_seed = gr.Number(label="Random Seed", value=42, precision=0)
                ...
                def _run_assign(manifest_file, num_raters, seed):
                    ...
                    for arm_name, arm_items in arms.items():
                        ...
                        lang_dist = _Counter(
                            it.get("language_id", "?") for it in arm_items
                        )
...
def _build_system_admin() -> gr.Blocks:
    ...
    with gr.Blocks() as tab:
        ...
        btn_refresh_health.click(refresh_health, outputs=[health_info, telemetry_info])
...
def _build_realtime_vc() -> gr.Blocks:
    ...
    with gr.Blocks() as tab:
        ...
        def do_convert(audio_path, char_id, energy, pitch_shift, speed):
            ...
            sr = resp.get("sample_rate", SAMPLE_RATE)
            audio_np = (
                np.frombuffer(out_bytes[44:], dtype=np.int16).astype(np.float32)
                / 32768.0
            )
...
def _build_onnx_export() -> gr.Blocks:
    ...
    with gr.Blocks() as tab:
        ...
        with gr.Row():
            src_uclm = gr.Textbox(
                label="UCLM Checkpoint", value="checkpoints/uclm_final.pt", max_lines=1
            )
            src_codec = gr.Textbox(
                label="Codec Checkpoint", value="checkpoints/codec.pt", max_lines=1
            )
    ...
    with gr.Blocks() as tab:
        ...
        with gr.Tabs():
            with gr.Tab("UCLM Training"):
                ...
                uclm_curriculum = gr.Textbox(
                    label="Curriculum Stage", interactive=False
                )
                ...
                for line in lines[-50:]:
                    ...
                    uclm_rows.append(
                        [
                            ...
                            d.get("loss", 0),
                            d.get("codec_loss", 0),
                            d.get("pointer_loss", 0),
                            d.get("progress_loss", 0),
                            d.get("adv_loss", 0),
                            "TTS" if d.get("mode", 0) else "VC",
                        ]
                    )
...
def _build_speaker_enrollment() -> gr.Blocks:
    ...
    with gr.Blocks() as tab:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                enroll_notes = gr.Textbox(
                    label="Notes",
                    placeholder="Voice characteristics, gender, age, accent...",
                    lines=2,
                )
        ...
        def remove_speaker(pid):
            ...
            rows = [
                [pid2, p.display_name, "uploaded", p.created_at]
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                test_method = gr.Radio(
                    choices=["GET", "POST"], value="GET", label="Method"
                )
                test_body = gr.Textbox(
                    label="Request Body (JSON)", lines=3, placeholder='{"key": "value"}'
                )
        ...
        with gr.Row():
            with gr.Column(scale=1):
                script_input = gr.Textbox(
                    ...
                    info=get_tooltip("batch_script"),
                )
                script_file = gr.File(
                    label="Or Upload Script (YAML/TXT)",
                    file_types=[".yaml", ".yml", ".txt"],
                )
                script_character = gr.Textbox(
                    label="Character ID",
                    value="default",
                    max_lines=1,
                    info=get_tooltip("batch_character"),
                )
            ...
            with gr.Column(scale=1):
                ...
                script_output_dir = gr.Textbox(
                    label="Output Directory",
                    value="exports/script_output",
                    max_lines=1,
                    info=get_tooltip("batch_output_dir"),
                )
                script_format = gr.Radio(
                    choices=["wav", "flac", "mp3"],
                    value="wav",
                    label="Format",
                    info=get_tooltip("batch_format"),
                )
                script_pace = gr.Slider(
                    0.5,
                    3.0,
                    value=1.0,
                    step=0.05,
...
def create_app() -> gr.Blocks:
    """Build the multilingual Gradio control plane with dynamic language switching."""

    with gr.Blocks(
        title="TMRVC Control Plane",
        theme=gr.themes.Soft(),
        head=get_head_js() + get_locales_js(),
    ) as app:
        lang_state = gr.State(value="ja")
        ...
        with gr.Row(elem_id="header"):
            ...
            lang_dropdown = gr.Dropdown(
                choices=[(lang["name"], lang["code"]) for lang in available_languages],
                value="ja",
                label="Language / 言語",
                scale=0,
                min_width=150,
        ...
        with gr.Tabs() as tabs:
            ...
            with gr.Tab("キュレーション", elem_id="tab-curation"):
                _build_curation_auditor()
            with gr.Tab("データセット", elem_id="tab-datasets"):
                _build_dataset_manager()
            with gr.Tab("A/B評価", elem_id="tab-eval"):
                _build_evaluation_arena()
            with gr.Tab("Eval Tools"):
                _build_eval_tools()
            with gr.Tab("キャスティング", elem_id="tab-casting"):
                _build_speaker_enrollment()
            with gr.Tab("Training Monitor"):
                _build_training_monitor()
            with gr.Tab("バッチ生成", elem_id="tab-batch"):
                _build_batch_script()
            with gr.Tab("ONNX Export"):
                _build_onnx_export()
            with gr.Tab("設定", elem_id="tab-settings"):
        ...
        def change_lang(new_lang):
            ...
            js = f"""
            <script>
                window.tmrvc_i18n.setLang('{new_lang}');
            </script>
            """
            return new_lang, js

        lang_output = gr.HTML(visible=False)

        lang_dropdown.change(
            change_lang,
            inputs=[lang_dropdown],
            outputs=[lang_state, lang_output],
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--server-url",
        default=None,
        help=f"tmrvc-serve API base URL (default: http://localhost:{SERVE_PORT})",
    )
    parser.add_argument("--port", type=int, default=GUI_PORT, help="Gradio port")
    ...
    _SERVER_URL = (
        ...
        if args.server_url
        else f"http://localhost:{SERVE_PORT}"
    )
