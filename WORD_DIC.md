...
| 用語 | 解説 |
|---|---|
| **TMRVC** | Unified Codec Language Model (UCLM) v2 を核とした、リアルタイム TTS (音声合成) および高精度 VC (音声変換) を実現する統合音声生成システム。 |
| **UCLM (Unified Codec Language Model)** | TTS と VC を単一のトランスフォーマー・アーキテクチャで統合した言語モデル。すべての音声生成を「条件付けされたトークン変換」として定義する。 |
...
| 用語 | 解説 |
|---|---|
| **UCLM v2** | TMRVC のメインエンジンとなるトランスフォーマー・モデル。音響トークン (`A_t`) と制御トークン (`B_t`) を同時に予測するデュアルヘッド構成を持つ。 |
| **Emotion-Aware Codec** | 感情表現や物理的な声の状態を保持したまま、音声をトークン化および復元するための高品質なニューラル・オーディオ・コーデック。 |
| **Dual-Stream Token Spec v2** | 音響情報（音色・フォルマント）を担う `A_t` と、非言語イベント（呼吸、強弱、タイミング）を担う `B_t` の2系統のトークンを並行して扱う仕様。 |
...
| **Mamba** | UCLM v2 の一部で使用されている状態空間モデル (SSM) アーキテクチャ。長いコンテキストを効率的に扱い、推論を高速化するために採用されている。 |
...
| **Polyphase Resampling** | 48kHz や 44.1kHz の音声をモデル内部の 24kHz に変換（およびその逆）するための効率的なサンプリングレート変換手法。 |
| **Ring Buffer** | 固定サイズの循環バッファ。音声データの入出力において、スレッド間のデータの受け渡しに使用される。 |
| **SPSC Queue** | Single-Producer Single-Consumer Queue。ロックフリーな通信を実現するためのキューで、オーディオスレッドと他のスレッドのやり取りに使用される。 |
...
| **DurationPredictor** | TTS モードにおいて、各音素が持続する時間（アライメント）を予測するモジュール。 |
