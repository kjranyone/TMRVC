# TMRVC — Disentangled UCLM Real-time Voice Conversion

## Project Overview

CPU-only で end-to-end 50ms 以下のリアルタイム Voice Conversion を実現する VST3 プラグイン。
**Disentangled UCLM パラダイム**: Emotion-Aware Codec + Disentangled Token Model (Transformer)。

- **内部サンプルレート:** 24kHz（DAW の 44.1/48kHz とはポリフェーズリサンプルで接続）
- **推論:** 6 ONNX モデルを ONNX Runtime で実行
- **学習パラダイム:** Disentangled UCLM (Emotion-Aware Codec + Transformer token model)
- **学習データ:** VCTK + JVS (T1) → LibriTTS-R (T2) → Emilia (T3) 段階的追加

## Architecture

```
DAW Audio In → VST3 Plugin → StreamingEngine (Rust) → ONNX Models ×6 → DAW Audio Out
                                     ↑
                               nih-plug 非依存
```

6 ONNX モデルの実行頻度:
- `codec_encoder` — per-frame (10ms ごと)
- `vc_encoder` — per-frame (10ms ごと、VC mode only)
- `voice_state_enc` — per-frame (10ms ごと)
- `uclm_core` — per-frame (10ms ごと)
- `codec_decoder` — per-frame (10ms ごと)
...
詳細は `docs/training/README.md` を参照。

### 2. Disentangled UCLM パラダイム (現行)

```
┌─────────────┐     ┌──────────────┐     ┌─────────────┐     ┌─────────────┐
│ Audio Input │ ──▶ │ Emotion-Aware│ ──▶ │ UCLM Core   │ ──▶ │ Codec       │ ──▶ Audio
│             │     │ Codec (Enc)  │     │ (Transformer│     │ Decoder     │
└─────────────┘     └──────────────┘     └─────────────┘     └─────────────┘
```

詳細は `docs/design/unified-codec-lm.md`, `docs/design/emotion-aware-codec.md` を参照。

### 3. Text-to-Speech (TTS) パイプライン
...
| File | Content |
|---|---|
| `architecture.md` | 全体アーキテクチャ、モジュール構成、主要設計判断、パラダイム移行履歴 |
| `unified-codec-lm.md` | UCLM (Unified Codec Language Model) アーキテクチャ詳細 |
| `emotion-aware-codec.md` | Token Spec v2 (`A_t`/`B_t`)、Emotion-Aware Codec 設計 |
...
| `rust-engine-design.md` | Rust エンジンクラス設計、TensorPool メモリレイアウト |
...
| `auth-spec.md` | 認証・ライセンス仕様 |

### docs/training/ (2 ファイル)

| File | Content |
|---|---|
| `README.md` | 学習パイプライン統合ガイド |
| `uclm-training-plan.md` | UCLM 学習計画 (Phase 1-4) |
