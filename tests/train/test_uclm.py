import torch
from tmrvc_core.constants import D_MODEL, RVQ_VOCAB_SIZE, CONTROL_VOCAB_SIZE
...
def test_voice_state_encoder():
    ...
    model = VoiceStateEncoder(d_model=D_MODEL)

    out = model(explicit, ssl)
    # VoiceStateEncoder may return tuple (state_cond, adv_logits) or just state_cond
    if isinstance(out, tuple):
        out = out[0]
    assert out.shape == (B, T, D_MODEL), f"VoiceStateEncoder shape mismatch: {out.shape}"
...
def test_vc_encoder():
    ...
    model = VCEncoder(d_model=D_MODEL, vq_bins=128)

    content_features, loss = model(source_tokens)
    assert content_features.shape == (B, T, D_MODEL), (
...
def test_uclm_transformer():
    ...
    assert logits_a.shape == (B, 8, T, RVQ_VOCAB_SIZE), (
        f"Transformer A_t shape mismatch: {logits_a.shape}"
    )
    assert logits_b.shape == (B, 4, T, CONTROL_VOCAB_SIZE), (
