...
class TestResidualVectorQuantizer:
    def test_forward_shape(self):
        ...
        assert quantized.shape == (1, 128, 1)
    ...
    def test_decode_shape(self):
        ...
        indices = torch.randint(0, 1024, (1, 4, 1))
...
class TestStreamingCodecEncoder:
    ...
    def test_state_persistence(self):
        ...
        state = encoder.init_state(1, torch.device("cpu"))
        x = torch.randn(1, 1, 480)

        _, state2 = encoder(x, state)
        _, state3 = encoder(x, state2)

        assert not torch.allclose(state, state2)
...
class TestStreamingCodec:
    ...
    def test_decode_shape(self):
        codec = StreamingCodec()
        indices = torch.randint(0, 1024, (1, 4))
