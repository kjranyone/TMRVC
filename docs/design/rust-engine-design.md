...
runtime claim は経路別に分離する。v3.0 で drama-grade CFG-enhanced acting claim を許可するのは Python serve path のみであり、Rust / VST は shared pointer/control contract と causal latency を主張対象とする。
...
2. raw conditioning (`explicit_voice_state`, `delta_voice_state`, optional `ssl_voice_state`) を準備する
...
3. target-side raw conditioning を準備する
4. `uclm_core` を回す
...
`pace`, `hold_bias`, `boundary_bias` は Rust runtime における primary timing-control path である。`voice_state` は同じ意味空間で解釈されるが、timing authority を二重化してはならない。
...
- v3.0 mainline では prompt-based `SpeakerProfile` のみを扱い、request-time weight hot-swap や adaptor merge は行わない
...
- prompt evidence が runtime budget を超える場合は、deterministic な圧縮または reject を行う
- benchmark / release-signoff 実行では silent degradation を許可せず、budget 超過は fail/invalid とする
...
### 8.4 explicit_voice_state / delta_voice_state as Explicit Inputs

v3 では `explicit_voice_state` と `delta_voice_state` を `state_cond` へ隠蔽せず、独立した制御入力として `uclm_core` に渡す。

- `explicit_voice_state`: `[1, 1, 8]` float32 — 現在の voice state (8 次元物理パラメータ)
- `delta_voice_state`: `[1, 1, 8]` float32 — 前 frame からの voice state 差分

engine は毎 frame、外部制御 (GUI スライダー、MIDI CC、オートメーション等) から `explicit_voice_state` を受け取り、前 frame との差分を `delta_voice_state` として計算する。これにより、推論中のリアルタイム voice state 操作が可能になる。

重要:

- runtime が受け取る `explicit_voice_state` は外部制御入力であり、学習側の `voice_state_targets` / mask / confidence 契約と同じ意味空間を持つ
- missing supervision を zero state と混同してはならない
...
### 8.5.1 Text Frontend Accept/Reject/Downgrade Parity

Rust runtime の text frontend は Python serve と同じ canonical contract に従う。

- canonical mainline target は `phoneme_ids` と optional `text_suprasegmentals`
- canonical `phoneme_ids` を十分な信頼度で生成できない request は、shared policy に従って downgrade または reject する
- fallback を受理する場合は `fallback_mode` を runtime telemetry / response metadata に残す
- Rust-only の grapheme/byte mainline contract を導入してはならない
- claim-valid な TTS runtime として出荷するには、Rust frontend が frozen Python frontend と同一 text に対して完全に同一の `phoneme_ids` と `text_suprasegmentals` を返すことを golden-fixture test で証明しなければならない
- segmentation, accent/tone extraction, punctuation/number normalization, code-switch routing, fallback classification のいずれも "ほぼ一致" では不十分であり、sign-off では exact parity を要求する

### 8.6 CFG Unconditional Mask Parity

Rust runtime は Python / ONNX と同じ unconditional mask 集合を使う。
...
この集合がずれる場合、`cfg_scale` の意味が runtime ごとに変わるため non-compliant とする。

benchmark / claim-valid run では CFG mode を固定し、budget 超過時に `full -> lazy/off` へ自動 downgrade してはならない。超過時は run を fail/invalid とする。real-time protective downgrade は通常運用時のみ許可される。
