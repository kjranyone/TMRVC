# TMRVC ONNX Model I/O Contract (v3 Pointer Mainline)
...
v3 の pointer state contract は section 3.2 で定義される。
v2 legacy の duration-based TTS は ONNX runtime contract には含まれない。
...
- canonical mainline TTS text contract is `phoneme_ids` plus optional `text_suprasegmentals`
- grapheme / byte fallback is downgrade-only and must not create a Rust/ONNX-only text contract
...
benchmark / claim-valid run では CFG mode を固定し、budget 超過時に `full -> lazy/off` へ silent downgrade してはならない。予算を超えた run は fail/invalid として扱う。
...
text frontend が canonical `phoneme_ids` を十分な信頼度で生成できない場合、runtime は request を downgrade または reject しなければならない。fallback を受理する場合でも `fallback_mode` を observable metadata として保持し、claim-valid run では silent fallback を許可しない。
...
v3.0 `SpeakerProfile` handoff は prompt evidence only を扱う。request-time weight mutation、LoRA/adaptor delta、model hot-swap 情報は canonical ONNX handoff に含めない。
...
- claim-valid run で budget 超過を silent downgrade で隠すこと
- canonical `phoneme_ids` を生成できない request を runtime 固有の text contract へ黙って変換すること
